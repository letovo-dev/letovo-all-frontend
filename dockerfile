...
ARG NEXT_PUBLIC_OTEL_DEPLOYMENT_ENVIRONMENT=
ARG NEXT_PUBLIC_OTEL_SERVICE_NAMESPACE=
ARG NEXT_PUBLIC_OTEL_TRACES_SAMPLER_RATIO=
...
RUN if [ -n "$NEXT_PUBLIC_BASE_URL" ]; then printf "\nNEXT_PUBLIC_BASE_URL='%s'\n" "$NEXT_PUBLIC_BASE_URL" >> ./.env; fi && \
    ...
    if [ -n "$NEXT_PUBLIC_OTEL_DEPLOYMENT_ENVIRONMENT" ]; then printf "NEXT_PUBLIC_OTEL_DEPLOYMENT_ENVIRONMENT='%s'\n" "$NEXT_PUBLIC_OTEL_DEPLOYMENT_ENVIRONMENT" >> ./.env; fi && \
    if [ -n "$NEXT_PUBLIC_OTEL_SERVICE_NAMESPACE" ]; then printf "NEXT_PUBLIC_OTEL_SERVICE_NAMESPACE='%s'\n" "$NEXT_PUBLIC_OTEL_SERVICE_NAMESPACE" >> ./.env; fi && \
    if [ -n "$NEXT_PUBLIC_OTEL_TRACES_SAMPLER_RATIO" ]; then printf "NEXT_PUBLIC_OTEL_TRACES_SAMPLER_RATIO='%s'\n" "$NEXT_PUBLIC_OTEL_TRACES_SAMPLER_RATIO" >> ./.env; fi && \
